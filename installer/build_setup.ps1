...
$AppDir = Join-Path $DistDir "Resumator 10"
$PayloadDir = Join-Path $InstallerDir "payload"
$PayloadZip = Join-Path $PayloadDir "Resumator10-app.zip"
$SetupSpec = Join-Path $ProjectDir "build-spec\Resumator 10 Setup.spec"
...
if (-not (Test-Path (Join-Path $AppDir "Resumator 10.exe"))) {
...
$SetupExe = Join-Path $DistDir "Resumator 10 Setup.exe"
